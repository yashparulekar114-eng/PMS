...
export default function GoalsPage() {
  ...
  // Calculate total and remaining weightage (Total capacity: 100%, Valid Target: 85%)
  ...
  const handleOpenModal = () => {
    setTitle("");
    setDescription("");
    // Default weightage to remaining needed for valid target or 15
  ...
  const handleAddGoal = async (e: React.FormEvent) => {
    ...
    // Enforce 100% maximum total weightage
    ...
    try {
      ...
      // If no cycle exists yet, create or get a default cycle to satisfy foreign key
      if (!targetCycleId) {
      ...
      // Insert new goal into Supabase with default status 'draft'
      const { error: insertError } = await supabase.from("goals").insert([
        {
          ...
          status: "draft", // Defaults status to 'draft'
        },
      ]);
      ...
      case "sent_back":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-100 text-rose-800 border border-rose-200">
            <AlertCircle className="w-3 h-3 text-rose-600" /> Sent Back
          </span>
        );
      case "draft":
      default:
                  ...
                  <tr key={goal.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-slate-700">
                      {goal.target_date ? (
                        ...
                        <span className="text-slate-400 italic text-xs">None set</span>
                      )}
                    </td>
